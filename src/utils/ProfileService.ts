...
constructor(gptRepo: ChatGPTRepository) {
  ...
  async getRandomProfile(){
    ...
    const profile = await this.gptRepo.callChatGPT('In JSON format create a beautiful name and a dating profile blurb limited to 150 words. full name and blurb must be different every time. properties must be name and blurb.')
